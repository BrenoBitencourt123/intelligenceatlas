...
import { Sparkles, Copy, Eye, EyeOff, ArrowRight, FileText, Award, Wand2, Loader2, Lock } from "lucide-react";
        ...
        <TabsContent value="summary" className="p-6 space-y-6 mt-0">
          ...
          <div className="text-center space-y-3 py-6 border-b border-border">
            <div className="inline-flex items-center justify-center">
              <div className="score-display text-5xl font-bold">{state.totalScore > 0 ? state.totalScore : "—"}</div>
            </div>
            <p className="text-sm text-muted-foreground">
              {state.totalScore > 0 ? "Nota estimada (ENEM)" : 'Clique em "Analisar redação" para ver a nota'}
            </p>
          </div>
            ...
            <div className="pt-4 space-y-4">
              {hasImprovedVersionAccess ? (
                ...
                <div className="text-center p-4 bg-muted/30 rounded-lg space-y-3">
                  ...
                  <p className="text-sm text-muted-foreground">Versão melhorada disponível no Plano Básico e Pro</p>
...
const CompetencyCard = ({ competency }: { competency: Competency }) => {
  ...
  return (
    <div className="p-4 bg-muted/30 rounded-lg space-y-2">
      <div className="flex items-center justify-between">
        <span className="font-medium text-sm text-foreground">{competency.name}</span>
        <span
          className={cn(
            "font-bold text-sm",
      ...
      {competency.score > 0 && <p className="text-xs text-foreground/80 pt-1">{competency.explanation}</p>}
    </div>
  );
};
...
const getNextSteps = (state: EssayState, hasImprovedVersionAccess: boolean): string[] => {
  ...
  if (emptyBlocks.length > 0) {
    steps.push(
      `Preencha ${emptyBlocks.length === 1 ? "o bloco" : "os blocos"}: ${emptyBlocks.map((b) => b.title).join(", ")}`,
    );
  }
